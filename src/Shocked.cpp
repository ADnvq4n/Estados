...
bool Shocked::cargar(const std::string& carpeta)
{
...
void Shocked::actualizar(float deltaTime)
{
    if (texturas.empty())
    {
        return;
...
void Shocked::dibujar(sf::RenderWindow& ventana)
{
    ventana.draw(sprite);
}

void Shocked::reiniciar()
{
    frameActual = 0;
    tiempoFrame = 0.0f;
